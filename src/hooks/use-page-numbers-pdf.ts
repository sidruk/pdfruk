...
export function usePageNumbersPdf() {
  ...
  const process = useCallback(async () => {
    if (!file) {
      setError("Add a PDF file to number.");
      return;
    }

    setIsProcessing(true);
    setProgress({ current: 0, total: file.pageCount, message: "Adding page numbers..." });
    setError(null);
    setResult(null);

    workerRef.current?.terminate();
    const worker = new Worker(
      new URL("../workers/page-numbers.worker.ts", import.meta.url),
    );
    ...
    worker.postMessage(
      { type: "page-numbers", buffer, options },
      [buffer],
    );
  }, [file, options]);
  ...
  return {
    ...
    removeFile,
    updateOption,
    setPosition: (position: PageNumberPosition) => updateOption("position", position),
    process,
    reset,
    canProcess: !!file && !isProcessing && options.format.trim().length > 0,
  };
}
